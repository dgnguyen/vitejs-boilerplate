...
import { errorPlayerSelector, getPlayersAction, IPlayer, loadingPagePlayerSelector, loadingPlayerSelector, playerDataSelector } from 'redux/reducers/player'
          ...
          {!loadingPage && data?.length > 0 && (
            ...
            >
              {data.map((row: IPlayer, ind: number) => (
                <Box key={`player-${row.playerId}`} className='playerRow-wrapper'>
                  <Box>{row?.playerId}</Box>
                  <Box>{row?.bcPlayerId}</Box>
                  <Box>{row?.agentName}</Box>
                  <Box className="transactionCount"><Typography>{row?.transactionCount}</Typography></Box>
                  <Box>{row?.totalBetAmount}</Box>
                  <Box>{row?.totalWinAmount}</Box>
                  <Box>{row?.ggr}</Box>
                  <Box>{row?.avgBetAmount}</Box>
                  <Box>{moment(row?.fistActivity).format(FORMAT_DATE_TIME)}</Box>
                  <Box>{moment(row?.lastActivity).format(FORMAT_DATE_TIME)}</Box>
                  <Box className={Boolean(row?.isTester) ? "test" : "real"}>{row?.isTester ? "Test" : "Real"}</Box>
